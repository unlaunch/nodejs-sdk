...
describe('Set Operators test', () => {
    // Invalid set attribute value

    it('Set AO, null userValue', function () {
        assert.throws(() => setApply("1,2,3",null, 'AO'), Error);
    })
    it('Set AO, userValue is not set type', function () {
        assert.throws(() => setApply("1,2,3",123, 'AO'), Error);
    })
    ...
    it('Set AO op: user set is superset should match', function () {
        assert.equal(setApply("1,2,3",new Set(["1", "2", "3", "4"]), 'AO'), true)
    })
    it('Set AO op: user set is subset should not match', function () {
